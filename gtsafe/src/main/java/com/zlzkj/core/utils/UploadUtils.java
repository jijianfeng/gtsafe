...
public class UploadUtils {
	...
	public static String basisOsChange(String path){
		String OS = System.getProperty("os.name").toLowerCase();//获取系统名称
		if(OS.indexOf("win") >= 0){
			path = path.replace("/", "\\");
		}
	...
	public static String getFileServer() {
		if(UploadUtils.getPathType().equals("relative")){
			return "/"+CoreUtils.ltrim(UploadUtils.getConfig("FILE_REPOSITORY"),"/");
		}
	 ...
	 * @return
	 */
	public static Map<String,Object> saveFileForApi(MultipartFile file,String... tokens){
		...
		if(!file.isEmpty()){
			...
			OutputStream out = null;
			String token = "";
			int NowTime = getNowTimestamp();
			if(tokens.length==1){
				token = tokens[0];
			}else{
				token = generateTokenForApi(file.getOriginalFilename(), String.valueOf(file.getSize()));
			}
			File f = null;
			try {
				in = file.getInputStream();
	 ...
	 * @return
	 */
	public static String generateTokenForApi(String name, String size){
		if (size == null){
			return null;
		}
		String token = "IMG_";
		token += getNowTimestamp();
		token += getExtName(name);
		return token;
	}
	 ...
	 * @return
	 */
	public static String getFileRepositoryForApi() {
		String path = "/imagelog";
	...
	/**
	 * API端把上传完成的临时文件，保存到相应的目录，
	 * 返回文件全路径
	 * @param file
	 * @param newSaveName
	 * @return
	 */
	public static String saveFileForApi(File file,String newSaveName){
	 ...
	 * @return
	 */
	public static String getFullSavePathForApi(String saveName){
		//获取文件保存目录
		String savePath = getFileRepositoryForApi();
		//获取当前年月日
		SimpleDateFormat sdf = new SimpleDateFormat("yyyyMMdd");
		String ymd = sdf.format(new Date());
		//获取文件种类
